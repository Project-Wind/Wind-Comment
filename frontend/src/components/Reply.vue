    ...
    <form class="w-full" @submit.prevent="postComment">
        <div class="flex justify-between flex-wrap flex-col sm:flex-row">
            <input required type="text"
                class="flex-1 rounded-md border-gray-300 shadow-sm focus:ring focus:ring-indigo-200 focus:ring-opacity-50 mb-2 sm:(mr-2 mb-0)"
                placeholder="称呼" v-model="name" />
            <input required type="email"
                class="flex-1 rounded-md border-gray-300 shadow-sm focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                placeholder="邮箱" v-model.trim="email" />
        </div>
        <textarea required
            ...
            rows="5" :placeholder="comment ? `回复 @${comment.name}：` : '请发表您的见解：'" v-model="content"></textarea>
        <button
            class="block mt-4 py-3 px-5 border border-gray-800 hover:(bg-gray-800 text-white) font-bold text-sm rounded-lg transition duration-150 ease-in-out">提交</button>
    </form>
</template>
