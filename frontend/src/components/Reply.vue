...
<template>
    <form class="w-full" @submit.prevent="postComment">
        <div class="flex justify-between flex-wrap flex-col sm:flex-row">
            <input required type="text" class="flex-1 rounded-md border-gray-300 shadow-sm focus:ring focus:ring-indigo-200 focus:ring-opacity-50 mb-2 sm:(mr-2 mb-0)" placeholder="称呼" v-model="name" />
            <input required type="email" class="flex-1 rounded-md border-gray-300 shadow-sm focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="邮箱" v-model.trim="email" />
        </div>
        <textarea required class="block resize-none placeholder-gray-500 mt-3 py-2 px-3 w-full rounded-md border-gray-300 shadow-sm focus:ring focus:ring-indigo-200 focus:ring-opacity-50" rows="5" :placeholder="comment ? `回复 @${comment.name}：` : '请发表您的见解：'" v-model="content"></textarea>
